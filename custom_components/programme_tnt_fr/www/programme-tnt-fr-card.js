...
var live = isProgLive(prog);
if (!live && prog.start) {
var key = channelLabel + "|" + prog.title + "|" + prog.start;
this._modalProg = { channelLabel: channelLabel, title: prog.title, start: prog.start, key: key };
...
"programme_tnt_fr",
"get_reminder_status",
{ channel_name: channelLabel, program_title: prog.title, start_time: prog.start },
undefined,
false,
true
)
.then(function (resp) {
if (!self2._modalProg || self2._modalProg.key !== key) return;
var scheduled = (resp && resp.response && resp.response.scheduled_minutes) || [];
var minutes = scheduled.length ? scheduled[0] : null;
self2._activeReminders[key] = minutes;
self2._applyReminderState(minutes);
})
.catch(function () {});
}
} else {
...
self._activeReminders[ctx.key] = minutes;
for (var bi = 0; bi < els.modalNotifyBtns.length; bi++) {
...
var els = this._els;
if (!ctx || !this._hass) return;
var minutes = this._activeReminders[ctx.key];
if (!minutes) return;
...
delete self._activeReminders[ctx.key];
for (var bi = 0; bi < els.modalNotifyBtns.length; bi++) {
