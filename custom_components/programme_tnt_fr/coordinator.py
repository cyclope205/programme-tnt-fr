...
class ProgrammeTntFrCoordinator(DataUpdateCoordinator):
    ...
    async def _fetch_and_parse(self) -> None:
        ...
        # Le flux complet (xmltv_fr.xml) pese plusieurs dizaines de Mo : le
        # parsing (CPU-bound) est deporte dans un thread pour ne pas bloquer
        # la boucle evenementielle de Home Assistant, et se fait en
        # streaming (ET.iterparse) plutot qu'en construisant l'arbre complet
        # en memoire, pour ne garder que les chaines suivies.
        channels_meta, programmes = await self.hass.async_add_executor_job(
            self._parse_xmltv_bytes, raw, set(self._channels)
        )
    ...
    def _parse_xmltv_bytes(
        ...
        """Parse le flux XMLTV en streaming (CPU-bound, execute dans un
        executor thread), en ne conservant que les chaines suivies plutot
        que de construire l'arbre complet (plusieurs dizaines de Mo, ~113k
        programmes) en memoire avant de filtrer.
        """
