...
SCHEDULE_SCHEMA = vol.Schema(_REMINDER_FIELDS)
CANCEL_SCHEMA = vol.Schema(_REMINDER_FIELDS)
...
def _reminder_id(channel_name: str, program_title: str, start_time: str, minutes_before: int) -> str:
    """Deterministic id derived from the identifying fields.

    Lets the card cancel a reminder using the same data it scheduled it
    with, without needing the server to hand back a generated id.
    """
    raw = f"{channel_name}|{program_title}|{start_time}|{minutes_before}"
...
class _ReminderManager:
    ...
    async def async_handle_schedule(self, call: ServiceCall) -> None:
        ...
        media_player_targets = self._resolve_media_player_targets()
        tts_engine = self._resolve_tts_engine()
        ...
        reminder_id = _reminder_id(channel_name, program_title, start_time_raw, minutes_before)
        reminder = {
            "id": reminder_id,
    ...
    async def async_handle_status(self, call: ServiceCall) -> ServiceResponse:
        ...
        scheduled = [
            ...
            if _reminder_id(channel_name, program_title, start_time, minutes) in existing_ids
        ]
    ...
    async def _async_notify(self, reminder: dict) -> None:
        ...
        for slug in targets:
            try:
                await self._hass.services.async_call(
                    "notify",
                    slug,
                    {"title": "Programme TNT FR", "message": message},
                )
            except Exception:  # noqa: BLE001
                _LOGGER.exception(
        ...
        if media_player_targets and tts_engine:
